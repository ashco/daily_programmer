...
function dataFormat (strng) {
  return strng
    .split('\n')
    .reduce((total, current) => {
      const [city, data] = current.split(':');
      const dataArr = data
        .replace(/[a-z]/gi, '')
        .split(',')
        .map(month => parseFloat(month));
      total[city] = dataArr;
      return total;
    }, {});
}
...
function mean(town, strng) {
  try {
    const townArr = dataFormat(strng)[town];
    const mean = townArr.reduce((total, current) => total += current, 0) / townArr.length;
    return mean;
  }
  catch (err) {
    return -1;
...
function variance(town, strng) {
  try {
    const meanNum = mean(town, strng);
    const townArr = dataFormat(strng)[town];
    const variance = townArr.reduce((total, current) => {
      return total += Math.pow(current - meanNum, 2);
    }, 0) / townArr.length;
    return variance;
  }
  catch (err) {
...
// console.log(dataFormat(data)['Doodara']);
console.log(mean("London", data)); // 51.19(9999999999996)
console.log(variance("DooDara", data)) // 57.42(833333333374)